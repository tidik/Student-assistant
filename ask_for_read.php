...
<div class="layui-body">
    ...
    <div style="padding: 15px;">
        ...
        <div class="layui-card layui-panel">
            <div class="layui-card-header"><div id="hint-msg" style="padding: 5px">当前类型：<span style="color:green">允许请假</span></div></div>
...
<script>
    layui.use(['table', 'dropdown','jquery','form'], function(){
        ...
        table.render({
            elem: '#test'
            ,url:'./controller.php?sign=getAsk&approve=1'
            ,toolbar: '#toolbarDemo'
                    ...
                    , click: function (obj) {
                        switch (obj.id) {
                            case 'approve':
                                ...
                                //当前类型：<span >允许请假</span>
                                $("#hint-msg").html('当前类型：<span style="color:green">允许请假</span>');
                                break;
                            case 'reject':
                                table.reload('test', {
                                    where: {
                                ...
                                $("#hint-msg").html('当前类型：<span style="color:red">拒绝请假</span>');
                                break;
                        }
                        layer.msg('数据重载成功');
                    }
                ...
                dropdown.render({
                    ...
                    click: function(menudata){
                        if(menudata.id === 'sendmail'){
                            $.post('./controller.php?sign=sendmsg', {UID:data.UID,sendtype:'email'}, function(str){
                                console.log(str);
                                if(str.code == 200){
                                    layer.msg(str.msg);
                                }
                            ...
                            layer.confirm('确定要发送短信通知 <b>'+ data.username +'</b> 的请假审批状态么?【运营商可能会收取一定费用】', function(index){
                                $.post('./controller.php?sign=sendmsg', {UID:data.UID,sendtype:'sms'}, function(str){
                                    console.log(str);
                                    if(str.code == 200){
                                        layer.msg(str.msg);
                                    }
